...
export function getWeekDateRange(date) {
  ...
  return { startDate: mondayUTC, endDate: sundayUTC, startString, endString };
}
...
/**
 * Updates the digital clock element in the header.
 * This is a global function, not exported.
 */
...
export async function initializeAppCore(pageSpecificInit) {
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      // --- Hide loader and show app wrapper immediately ---
      ...
      try {
        ...
        // --- NEW: ATTACH GLOBAL LISTENERS & START CLOCK ---
        ...
        // --- Call page-specific init *after* profile is loaded ---
...
async function loadPreferences(globalRef, adminRef, userRef) {
  try {
    ...
    // Allow the page to load with defaults if preferences fail
    ...
    if (item.subItems && Array.isArray(item.subItems)) {
      ...
      toggleButton.innerHTML = `
        <span class="flex items-center gap-4">
          <i class="fas ${item.icon} fa-fw w-6"></i><span>${item.text}</span>
        </span>
...
function attachCoreEventListeners() {
  // Use event delegation on the document body for header/sidebar elements
